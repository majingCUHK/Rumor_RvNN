...
@author: majing
@module: propagation tree, recursive-NN model
@variable: Nepoch, lr
@time: Jan 24, 2018
"""
...
Nepoch = 600
lr = 0.005
obj = "Twitter15" # dataset
tag = "_u2b"#"_Htime"
fold = "2"

unit="treeGRU-pool-"+obj+str(fold)+'-vol.'+str(vocabulary_size)+tag
#lossPath = "../loss/loss-"+unit+".txt"
#modelPath = "../param/param-"+unit+".npz" 

treePath = '../resource_tree/tree2rnn.post.vol_'+str(vocabulary_size)+tag+'.txt' 

trainPath = "../nfold/RNNtrainSet_"+obj+str(fold)+"_tree.txt" 
testPath = "../nfold/RNNtestSet_"+obj+str(fold)+"_tree.txt"
labelPath = "../resource_tree/"+obj+"_label_All.txt"
...
## 2. ini RNN model
t0 = time.time()
model = tree_gru_u2b_pool.TreeRNN(vocabulary_size, hidden_dim, Nclass)
t1 = time.time()
